...
namespace mongo {
    ...
    void QueryPlanGenerator::addFallbackPlans() {
        ...
        vector<shared_ptr<QueryPlan> > specialPlans;
        ...
        // TODO Harish: worst possible handling of special plans
        // And I am making it even worse
        
        // Irrespective of other indexes, if a kdtree is present and is a HELPFUL plan, then use it
        // It cannot be much worse than btree plans anyway
        for( vector<shared_ptr<QueryPlan> >::const_iterator i = specialPlans.begin(); i != specialPlans.end(); ++i ) {
        	if((i->get())->special() == IndexNames::KDTREE) {
        		_qps.setSinglePlan( *i );
        		return;
        	}
        }
    ...
    bool QueryPlanGenerator::addSpecialPlan( NamespaceDetails* d ) {
        ...
        SpecialIndices special = _qps.frsp().getSpecial();
        {
            ...
            while( i.more() ) {
                ...
                if (pluginName == IndexNames::KDTREE && 
                		(USELESS != IndexSelection::isSuitableFor(keyPattern,_qps.frsp().frsForIndex(d, j), _qps.order()))) {
...
#define GUARD_RUNNER_EXCEPTION( runner, expression ) \
    ...
    void MultiPlanScanner::init( const BSONObj& order, const BSONObj& min, const BSONObj& max ) {
        ...
        // Ugly hack to support kdtree for $or queries. Out of time to figure out the "right" way to do it.
        if(_ns != "local.system.indexes") {
            ...
            while( i.more() ) {
                ...
                string pluginName = CatalogHack::getAccessMethodName(keyPattern);
                if(pluginName == IndexNames::KDTREE) {
                	if(USELESS != IndexSelection::isKdTreeSuitableFor(keyPattern, _query)) {
                		_or = false;
                	}
                	break;
                }
